...
C  DATA SAMPLE IS IN BUF(61).   IF THE DATA IS 16 BIT INTEGER ON TAPE, RDTRC
...
C  CALLING PROGRAM.  THE OPERATOR CAN HANDLE THIS THROUGH FILE WETALK.  SEE
C  SUBROUTINE TPCHNG.
C     THE WAY TO USE THIS PROGRAM IS TO JUST KEEP CALL RDTRC UNTIL THE USER
C  WANTS TO STOP, OR THE OPERATOR STOPS THE JOB THROUGH TPCHNG.  THE CALLING
C  PROGRAM SHOULD CHECK EACH TRACE HEADER TO SEE IF THE TRACE IS LIVE OR DEAD, A
C  DATA TRACE OR AN AUXILIARY ETC.  SEE THE ARTICLE IN GEOPHYSICS FOR THE
...
c      IF( icompt .EQ. 2 .OR. icompt .EQ. 4 ) CALL swap16(ibuf,200)
  ...
  100 CONTINUE
      ...
      IF(ISIG.EQ.2.OR.ISIG.EQ.3) IFUNC=40                               ! WAIT FOR I/O COMPLETION AND RETURN STATUS
