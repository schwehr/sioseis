 ...
 * PODISCB64(lun, mode, int_64(nbyte)) positions lun to byte 64bit nbyte
 ...
 * FILSIZ( name, int_64(isize) )  return the size of the file name
 ...
 * ADRDISC64( lun, int_64(address)) Return the current disk address (2 32bit words).
...
mod 12 Oct 10 - Linux now defines   long   as 64 bits, so explicitly specify
                the word size on all arguments from fortran.
mod 5 Feb 14 - change NULL to 0  on:  if (( *istat == chdir( dir ) ) == NULL) return;
mod 13 Apr 15 - podiscb64 didn't work
...

{
	...
	printf("getfil, mode= %d\n",*mode);
      ...
      if(*mode < 0 ) {   /* negative mode means the caller specifies the lun */
		...
		if( *mode == -4 || *mode == -5 )  {
			strcpy(tname,name);
			if( *mode == -4 ) mknamec( tname ); /* make sure the name terminates with a NULL */
			status = open ( tname, 2);
			if( status == -1 ) status = open( tname, 0);
			if( status == -1 ) status = open( tname, 1);
			if( status == -1 ) {
...
#if debug
	printf(" rddisc, lun=%d, from byte %d offset=%d nbytes=%d, status=%d\n",*lun,offset,nbytes,status);
#endif
       if( status < 0 ) {
