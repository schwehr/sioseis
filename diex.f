...
c
c  MODIFICATIONS:
c  18 Jul 18 - Convert ODEC nav to arcseconds from floating point
c  5 Nov 15 - Added FORMAT NIUST
c  11 Apr 15 - Add iaddr64 for 64 bit disc address for realtime and random
c  8 Jul 14  - Clean up shot/rp searches when fno/lno are given, especially when
c                the shot numbers are not increasing.
c            - Add a "suggestion" when nothing is found.
c            - Rev 1 random access didn't work on MCS (ntraces > 1)
c  June 14   - Use subroutine swp_tr for trace header byte swaps so it's consistent with disko
c  31 Mar 14 - Add error if trinc is used when process SORT is used.
c            - add si to common/readt/ - will eventually remove all ref to buf(49)
c            - add segyrev to common/readt/
c            - Allow Rev 1 scalar for delay
c            - set ibuf(108) to 0 if rev 0 so that disko doesn't pass it with rev 1
c            - make sure the fixed file flag is set to 0 on rev 0
...
c****     ODEC & SWAPPED are in little endian (little byte first)
     ...
     &                segyrev
              ...
              segyrev = 0
          ENDIF
c****     watch out for rev 0 files that disko will convert to rev 1
...
c**** Random access requires the user to give FNO
      IF( random .NE. 0 .AND. fno .NE. 0 ) THEN
c****   watch out for 32 bit arithmetic, so force 64
...
c  trace number rather than the shot number, so swap the meaning!
          lbuf(3) = lbuf(2)
          lbuf(4) = lbuf(1)
c****        ibuf(45) = 3        !  the nav are IEEE floating decimal degrees - I want code 5!!!!
          ibuf(36) = -100
...
c****    retrac other than 1 will cause the following to cause havic
     ...
     &            ( ltr .NE. 0 .AND. lbuf(4) .EQ. ltr )) THEN
      ...
      IF( jform .EQ. NIUST ) THEN
          ...
          CALL swap16( ibuf(91),1)
          CALL swap32( lbuf(46),1)
c          lbuf(4) = 1            ! just in case the user doesn't do it
          segyrev = 1.
c          ibinhdr(151) = 256     ! doex automatically sets it to 256 also so don't do it
          ifixed = 1
c          ibinhdr(152) = ifixed  ! doex does this also so don't do it
      ENDIF
      ...
      IF( segyrev .GE. 1 .AND. ibuf(108) .NE. 0 ) THEN
          ...
          ENDIF
      ELSE
c****     disko will try to write rev 1 and if 108 is non-zero then the delay scalar is set!
c****        (Knudsen rev 0 uses 108.  Subsequent sioseis processing will make the output rev 1)
          ibuf(108) = 0
      ENDIF
