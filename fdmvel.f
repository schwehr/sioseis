...
c
c changes:
c mod 24 Mar 2015 - handle dt < 1 mil
c
c
C-------------------------------------------------------------------------------
...
c  June 2000 - g77 didn't like float((dt*1000.) because it was a real
c  9 June 2004 - Increase apdata to 5000000
c 14 Aug 2007 - g95 requires array indices must be integer
c
...
c          velint = sqrt(veloc(lenint/sr))
c          velint = sqrt(veloc(lenint/(dt*1000.)))
c          itemp = dt*1000.
c          velint = sqrt(veloc(lenint/itemp))
...
c               sc = tc / float(sr) + 1.0                                ! Sample number
