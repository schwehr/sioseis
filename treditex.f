...
c  Mod 17 Jun 14 - Do the offline filter for ALL pickers
c                - Use frequency filter with dbdrop 48 rathe than time domain filter
c
c
...
c              ldummy(1) = 1
c              ldummy(2) = numdat
c              CALL tvfilt( apdata(nextad), scr, apdata(nextad+numdat),
c     &             filpts, nfilpts, 1., ldummy, 1, numdat )
c          ENDIF
          ave1 = 0.
          DO i = is, ie
             ave1 = ave1 + ABS(scr(i))
