      SUBROUTINE NMOEX(BUF,LBUF,IBUF,SCR,LSCR,ISCR, nmonum )
...
c  mod 2 Apr 15 - HIRES problem (mixing time and indices)
c               - HIRES was garbage when data had delay
c               - HIRES indexing was off
c  mod 18 Jul 18 - Swap the velocity values when writing a segy OPATH
c                - Also don't requite lprint to be used to make OPATH to work
C
      PARAMETER ( NPARS = 18 )
...
c****     swap it back for the next usage
              IF( icompt .EQ. 2 .OR. icompt .EQ. 4)
     *            CALL swp_trhdr( ibuf, lbuf )
              IF( icompt .EQ. 2 .OR. icompt .EQ. 4)
...
c  a(indata+1) has the trace
                 IF( a(nextad+i-n2) .GT. a(nextad) ) THEN
                     CALL polint( a(indextx+i-n2),          ! a(indextx) is the first
     &                            a(indata+1+lscr(i-n2)),   ! lscr(1) is the first & indata+1 is first sample
     &                            nterpolate, a(nextad+i),   ! a(nextad) is the first
     &                            buf(numhdr+i), temp )    ! buf(numhdr+1) is the first
                 ELSE
	IF( ipaul == 0 ) Then
            print *,' notify Paul - hires nmo weirdness'
            print *, ' send data and script.'
        endif
        ipaul = 1
        buf(numhdr+i) = 0.
c                     CALL polint( txtimes, a(indata+lscr(i-1)),
c     &                 nterpolate, a(nextad+i-n2),
c     &                    buf(numhdr+i), temp)
                 ENDIF
             ENDIF
          ENDDO
