 ...
 *
 *
 *      mtegt.mt_dsreg  (drive status register):
 *    ************     Doesn't exist on HP    *******************
 *             EMULEX CONTROLLER
...

{
      ...
      sprintf(device, "/dev/nst%d\0",unitno);
...
*/
        switch( *n ) {
                ...
                case 5:
                        *stat = mtget.mt_blkno;
                        break;
/*   mt_flags  and mt_bf   not in Mac osx
                case 6:
                        *stat = mtget.mt_flags;
                        break;
                case 7:
